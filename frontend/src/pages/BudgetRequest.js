...
export default function BudgetRequest() {
  ...
  const isApprover = user?.role?.toLowerCase() === 'director';

  const fetchData = useCallback(async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${API_URL}/budget-requests`, {
...
function BudgetLineItemRow({ item, index, skus, cities, eventCity, onUpdate, onRemove }) {
  ...
  const selectLead = (lead) => {
    onUpdate(index, 'lead_id', lead.id);
