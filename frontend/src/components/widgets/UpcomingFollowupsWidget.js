    ...
    <Card className="overflow-hidden border-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl shadow-lg shadow-slate-200/50 dark:shadow-slate-900/50">
      ...
      <div className="p-4 sm:p-5 pb-3 flex items-center justify-between">
        <h2 className="text-sm font-semibold text-slate-800 dark:text-white flex items-center gap-2">
          ...
          <span className="hidden sm:inline">Upcoming</span> Follow-ups
        </h2>
        <Button 
          variant="ghost" 
          size="sm" 
      ...
      <div className="px-4 sm:px-5 pb-4 sm:pb-5">
        {upcomingLeads?.length > 0 ? (
            ...
            {upcomingLeads.slice(0, 4).map(item => {
              ...
              return (
                <div
                  ...
                  className="flex items-center gap-2 sm:gap-3 p-2.5 sm:p-3 rounded-xl border border-slate-100 dark:border-slate-800 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:border-slate-200 dark:hover:border-slate-700 transition-all duration-200 group"
                >
                  <div className={`p-1.5 sm:p-2 rounded-lg ${item.type === 'account' ? 'bg-emerald-100 dark:bg-emerald-900/30' : 'bg-blue-100 dark:bg-blue-900/30'}`}>
                    <Building2 className={`h-3.5 w-3.5 sm:h-4 sm:w-4 ${item.type === 'account' ? 'text-emerald-600 dark:text-emerald-400' : 'text-blue-600 dark:text-blue-400'}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-xs sm:text-sm text-slate-800 dark:text-white truncate group-hover:text-primary transition-colors">
                  ...
                  <Badge variant="secondary" className={`text-[10px] sm:text-xs shrink-0 px-1.5 sm:px-2 ${dateInfo.color}`}>
                    {dateInfo.text}
                  </Badge>
                </div>
              );
          ...
          <div className="text-center py-6 sm:py-8">
            <CalendarDays className="h-8 w-8 sm:h-10 sm:w-10 text-slate-200 dark:text-slate-700 mx-auto mb-2" />
            <p className="text-muted-foreground text-xs sm:text-sm">No follow-ups this week</p>
          </div>
        )}
      </div>
    </Card>
