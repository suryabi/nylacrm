...
import {
  Sparkles, X, Send, Loader2, ThumbsUp, ThumbsDown, BookOpen, RefreshCw,
} from 'lucide-react';
    ...
    <>
      ...
      {!isOpen && (
        <button
          ...
          className="fixed bottom-6 right-6 z-50 group flex items-center gap-2 pl-3 pr-4 py-3 rounded-full bg-gradient-to-br from-violet-600 via-fuchsia-500 to-pink-500 text-white shadow-lg shadow-violet-300/40 hover:shadow-xl hover:shadow-violet-300/60 hover:-translate-y-0.5 transition-all duration-200 ring-1 ring-white/30"
        ...
        >
          ...
          <span className="text-sm font-semibold tracking-tight">Ask Nyla</span>
        </button>
      )}

      {/* Chat panel */}
      {isOpen && (
        <div className="fixed bottom-6 right-6 z-50 w-[380px] sm:w-[440px] max-h-[80vh] flex flex-col rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-2xl overflow-hidden" data-testid="ask-nyla-panel">
          {/* Header */}
          <div className="relative flex items-center justify-between p-4 bg-gradient-to-br from-violet-600 via-fuchsia-500 to-pink-500 text-white">
