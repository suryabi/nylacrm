...
import { Calendar, Clock, Plus, Video, ExternalLink } from 'lucide-react';
        ...
        {upcomingMeetings?.length > 0 ? (
          <div className="space-y-2">
            {upcomingMeetings.slice(0, 3).map(meeting => (
              <div key={meeting.id} className="p-3 bg-white/10 backdrop-blur rounded-lg border border-white/20 hover:bg-white/20 transition-colors">
                <div className="flex items-start justify-between gap-2">
                  <p className="font-medium text-sm text-white truncate flex-1">{meeting.title}</p>
                  {meeting.meeting_link && (
                    <button
                      onClick={(e) => handleJoinMeeting(e, meeting.meeting_link)}
                      className="flex items-center gap-1 px-2 py-1 bg-white/20 hover:bg-white/30 rounded text-xs text-white font-medium transition-colors"
                    >
                ...
                {meeting.zoom_password && (
                  <p className="text-xs text-white/60 mt-1">Password: {meeting.zoom_password}</p>
                )}
