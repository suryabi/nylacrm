...
export default function LeadRankingTiles({ currentRank, onRankChange, disabled = false, saving = false }) {
  ...
  return (
    <div className="flex gap-3" data-testid="lead-ranking-tiles">
      {ranks.map((rank) => {
        ...
        return (
          <button
            ...
            disabled={disabled}
            className={cn(
              "relative flex-1 flex items-center justify-center gap-3 px-4 py-3 rounded-lg border-2 transition-colors duration-150",
              "focus:outline-none",
              isSelected 
