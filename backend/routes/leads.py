...
async def get_lead_bottle_designs(lead_id: str, current_user: dict = Depends(get_current_user)):
    ...
    tdb = get_tdb()
    lead = await tdb.leads.find_one({'$or': [{'id': lead_id}, {'lead_id': lead_id}]}, {'_id': 0, 'id': 1, 'bottle_designs': 1, 'company': 1, 'logo_url': 1, 'current_volume': 1, 'opportunity_estimation': 1})
    if not lead:
        raise HTTPException(status_code=404, detail='Lead not found')
    ...
    monthly_bottles = est.get('monthly_bottles') or est.get('final_monthly') or est.get('calculated_monthly')
    return {'designs': designs, 'company': lead.get('company'), 'has_logo': bool(lead.get('logo_url')), 'lead_uuid': lead.get('id'), 'current_volume': lead.get('current_volume'), 'monthly_bottles': monthly_bottles}
