...
- [x] **Plan Optional + Leads Navigation (2026-04-01)**: Target Plan made optional in Performance Tracker — users can generate metrics without a plan (target=₹0). New `/api/performance/all-sales-resources` endpoint loads resources independently. Leads Targeting Next Month click passes all filters (assigned_to with ALL resource IDs, target_closure_month/year). Backend get_leads now accepts target_closure_month/year params. LeadsList reads comma-separated assigned_to from URL. Amber banner shows active target closure filter. Filter layout reorganized into 2 rows for better spacing.
