...
- **Email** (`utils/email_sender.py`, `utils/notify.py`): best-effort `send_email` that uses Google Workspace / generic **SMTP** if `SMTP_HOST/USER/PASSWORD` are set, else falls back to **Resend** (already configured: `RESEND_API_KEY` + `SENDER_EMAIL=noreply@nylaairwater.earth`). `notify_users()` writes per-recipient in-app notifications and emails them. Added `.env` placeholders: `APP_BASE_URL` (for clickable absolute email links), `SMTP_HOST/PORT/USER/PASSWORD`.
...
- **Note**: To send email from your own Google Workspace domain instead of Resend, add `SMTP_HOST=smtp.gmail.com`, `SMTP_PORT=587`, `SMTP_USER=<workspace mailbox>`, `SMTP_PASSWORD=<Google App Password>` (needs 2-Step Verification on that account). Set `APP_BASE_URL=https://crm.nylaairwater.earth` for clickable links in emails.
