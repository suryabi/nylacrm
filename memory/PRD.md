...
- [x] **Fix frontend** `/app/frontend/src/components/distributor/SettlementsTab.jsx` — derives net_settlement from `s.stockout_totals` (`net_billable − billed_at_transfer − factory_return_credit`) instead of stored `settlement.factory_distributor_adjustment`. Same change applied to grand-total tile, account-group tile, per-row table, and Excel export (which now also includes the `Already Billed at Transfer` column).
- [x] **Verified** via curl: STL-2026-0017 → list endpoint NET = ₹4,611.00 ≡ detail endpoint NET = ₹4,611.00 (MATCH ✓). Old stale stored value (~₹4,971) is no longer surfaced. Lint clean.
...
- [x] **NextMonthLeadsSubsection** renders violet pill banner, helper text 'Active leads with target closure in {Month Year}', 3 summary stats (Leads Count / Pipeline Value / Filtered Value), search box (filters by lead/company/city/status), 'Open in Leads' deep link, and a sticky-left 5-column table (Lead / Company / City / Status w/ colored dot / Pipeline Value) with zebra striping, totals footer, and clickable rows that navigate to `/leads/{lead_id}`.
- [x] **Backend** `GET /api/performance/generate` now returns `pipeline.next_month_leads_list[]` with `{id, name, company, city, status, pipeline_value, target_closure_month, target_closure_year}` — sums consistent with existing `next_month_leads_count` / `next_month_pipeline_value`.
- [x] **Files**: `backend/routes/performance.py` (added next_month_leads_list to generate response), `frontend/src/pages/PerformanceTracker.js` (OverridableTile, NextMonthLeadsSubsection, summary tile row rewrite, removed Revenue Metrics section, removed Pipeline-section CTA, Top10PrioritiesSection accepts nextMonthLeads/nextMonth/nextYear/nextMonthPipelineValue).
- [x] **Testing**: iteration_160 — 7/7 backend pytest PASS (`backend/tests/test_performance_next_month_leads.py`) + frontend Playwright verified end-to-end. Stale test ids confirmed removed: `revenue-section`, `revenue-lifetime`, `revenue-this-month`, `revenue-new-accounts`, `leads-targeting-next-month-link`, `metric-pipeline`. Override edit→save→amber styling round-trip verified.
...
### RBAC Permission Matrix — Full Menu Coverage (2026-04-29)
- [x] **Backend** `/app/backend/models/role.py` extended: `DEFAULT_MODULE_PERMISSIONS` 47 → 71 keys; `MODULE_CATEGORIES` 11 → 12 categories; `MODULE_LABELS` updated to map every key. Added: `account_gop_metrics, neck_tag_designer, invoices, performance_tracker, investor_dashboard, meeting_minutes, cogs_components, api_keys, sku_replace, production_dashboard, production_batches, qc_routes, qc_team, rejection_reasons, rejection_report, rejection_cost_config, packaging_types, stock_dashboard, cost_cards`.
- [x] **Auto-backfill** in `routes/roles.py::list_roles`: existing roles get missing module keys merged in on read — `Admin`/`System Admin` roles get `{view,create,edit,delete}=true`, all other roles default to `false` (least-privilege). Persisted to DB so subsequent reads are stable.
- [x] **Frontend** `/app/frontend/src/layouts/DashboardLayout.js` — added/corrected `moduleKey` on every sidebar item that lacked one: Account GOP Metrics, Neck Tag Designer, Lead Scoring Model (`lead_scoring` → `lead_scoring_model` to match backend), Tenant Settings, API Keys, Replace SKU, all 4 "Tasks & Requests" instances, all 8 Production submenu items (Dashboard, Production Batches, QC Routes, QC Team, Rejection Reasons, Rejection Report, Rejection Cost Config, Packaging Types), Distribution Stock Dashboard / Cost Cards (split off from generic `distributors`).
- [x] **Result**: Tenant Settings → Roles tab now exposes 12 categories with correct counts (Core 10 / Reports 5 / Operations 7 / Tools 5 / Documents 2 / Requests 3 / Marketing 2 / Organization 8 / Admin 3 / Distribution 10 / Production 13 / Task Management 3 = 71 modules) and every sidebar route is gateable per role.
- [x] **Testing**: iteration_153 — 10/10 backend pytest + frontend Playwright PASS. Verified all 19 new keys present, backfill behaviour for admin vs non-admin roles, PUT /api/roles/{id} persists toggles, /api/auth/me regression clean, Tenant Settings UI renders all categories.

### Personal Calendar — Cherry-picked from prod_qc_sync fork (2026-04-29)
- [x] **New per-user calendar** that aggregates CRM meetings, meeting_minutes, and (optionally) Google Calendar events into a unified Day/Week/Month view at `/personal-calendar`.
- [x] **Backend** new module `backend/routes/personal_calendar.py`: `GET /google/status`, `GET /google/connect` (OAuth redirect), `GET /google/callback`, `POST /google/disconnect`, `GET /events?start_date&end_date`, `POST /google/push-meeting/{meeting_id}`. Per-user OAuth tokens stored in `user_google_tokens` collection on global db.
- [x] **Frontend** new `frontend/src/pages/PersonalCalendar.js` (837 lines): Day/Week/Month toggle, "New Meeting" modal, "Connect Google Calendar" CTA, color legend (sky=CRM Meeting, violet=Minutes, red=Google), event pills per day cell.
- [x] **UpcomingMeetingsWidget.js** updated from prod_qc_sync (212 insertions / 113 deletions) to integrate with the new calendar data source.
- [x] **Integration**: added router include in `backend/routes/__init__.py`, `google-auth-oauthlib==1.3.1` to requirements, `PersonalCalendar` route + import in `App.js`, "My Calendar" sidebar item under Lead & Sales Operations with `moduleKey: 'personal_calendar'`.
- [x] **RBAC**: `personal_calendar` added to `DEFAULT_MODULE_PERMISSIONS`, `MODULE_LABELS` ("My Calendar"), and `MODULE_CATEGORIES.Operations`. Total module keys 71 → 72; Operations 7 → 8. Existing roles get backfilled on next read.
- [x] **Cherry-pick strategy**: pulled NEW files (`personal_calendar.py`, `PersonalCalendar.js`, `UpcomingMeetingsWidget.js`, test file) from `upstream/prod_qc_sync` via `git checkout`, then surgically merged 5 integration points via `search_replace` to preserve all the rejection-cost-dash work (production_qc.py, ProductionDashboard.js, ProductionBatches.js, BatchDetail.js, RejectionCostConfig.js, RejectionReport.js, role.py, roles.py) which was newer/more complete on this branch.
