...
- **Fix** (`routes/distributors.py` `get_stock_dashboard._item_crates`): branch on the `packages` field — if `packages > 0`, `quantity` is already the base total → use it as-is; if no `packages` and `packaging_units > 1` (legacy), fall back to `quantity × packaging_units`; else use `quantity`. Added `packages` to the shipment/delivery/reserved/recent projections and used the helper for the weekly average, so all four aggregations stay in lock-step. RECEIVED path preserved.
- **No data migration needed** — the stored data is correct and self-consistent; only the aggregation was wrong. Fix is code-only; redeploy to production applies it.
- **Verified**: distributor bb12d90e — Sparkling DELIVERED = 24 (was 288 with two qty=12/pu=12/pk=1 completed deliveries); RESERVED/RECEIVED intact; frontend Stock Dashboard + Bottles/Crates toggle consistent.

## 2026-07-07 — ✨ Stock-by-SKU bottles/crates toggle + 🐛 Fix: Factory→Master Warehouse crates-as-bottles + 🎨 Lead/Account detail redesign ✅ (testing_agent iteration_309)
- **Stock-by-SKU unit toggle** (`components/distributor/StockDashboardTab.jsx`): the "Stock by SKU" table now has a **Bottles / Default Crates** toggle (`data-testid="stock-unit-toggle"`). Every quantity column (Received, Delivered, Reserved, Empty, Returns, Wh Stock, Available, Wkly Avg) converts to each SKU's **own** default crate (`bottles ÷ default_packaging_units`). The crate size is always shown per row ("1 Crate - 12 = 12 Bottle") and the alternate unit appears as an ≈ subtext. SKUs with no multi-unit crate (base UOM only, e.g. Sparkling/20L) stay in bottles. Verified: 600ml Silver 12,216 bottles ↔ 1,018 crates.
- **🐛 Factory → Master Warehouse crates-as-bottles (P0)**: root cause — some SKUs/batches had `bottles_per_crate` unset (A330/Nyla 330 Silver master SKU was `None`), so `BatchDetail.js` "Transfer to Master Warehouse" fell back to `×1` (200 crates stored as 200 bottles). Fix (`routes/production_qc.py`): added `_sku_crate_units` + `_effective_bottles_per_crate`; `GET /production/batches/{id}` now always returns a correct crate size (derived from the SKU packaging when the batch value is missing), and `transfer_to_warehouse` stores the effective crate size on factory rows. Added idempotent backfill `scripts/backfill_bottles_per_crate.py` (fixed 5 master SKUs + 8 factory-stock rows in preview). Verified: a null-bpc batch now returns 24. **Run the backfill script on production after deploy.**
- **🎨 Lead & Account Detail redesign (P0)**: added a sticky "On this page" table-of-contents nav (`StickySectionNav` + `useActiveSection`, xl-only) and distinct section headers (`SectionHeader`: eyebrow + icon tile + emerald accent) to `LeadDetail.js` (12 sections) and `AccountDetail.js` (9 sections). Sections wrapped with anchor ids; inner data/form logic untouched. Fixed testing-agent regressions: dead `lead-contact` & `acc-location` anchors re-added, and active nav pill now updates immediately on click.
- New reusable components: `components/detail/Section.jsx`, `SectionHeader.jsx`, `StickySectionNav.jsx`.
...
### 2026-07-05 — Customer Branding moves from Lead → Account on conversion ✅ DONE (self-verified, both pages)
- The `LeadBottleDesigns` "Customer branding" card (bottle designs + design requests, full create+view) now also renders on the **Account** detail page (`AccountDetail.js`, after the Contacts section) whenever `account.lead_id` is set — referencing the source lead (single source of truth, stays in sync; also still visible on the Lead).
- Robustness: `account.lead_id` can be the **formatted** lead id (e.g. THER-HYD-L26-001) rather than the UUID. `GET /leads/{id}/bottle-designs` now resolves by `{id OR lead_id}` and returns `lead_uuid` + `has_logo`; `LeadBottleDesigns` uses the resolved UUID for the requests list, delete, bottle-preview and all from-lead create endpoints, and self-determines `hasLogo` from the lead. Verified: account "The Rameshwaram Cafe" shows the branding card with linked request MR-2026-0017; Lead page unchanged (no regression).
...
- **Lint cleanup (high-traffic pages)**: resolved the `react-hooks/exhaustive-deps` warnings (via the sanctioned `// eslint-disable-next-line` escape hatch for intentional mount/param-driven fetch effects — zero behavioral change) across the 9 busiest pages: HomeDashboard, Dashboard, LeadsList, LeadDetail, AccountDetail (×2), ContactsList, MarketingRequestDetail, DesignRequestNewDetail, DistributorDetail. Verified with an ESLint-forced build: warnings dropped **58 → 47** and all 9 files are now clean, no new warnings. (Build-time ESLint stays disabled via `.env` as the deploy safety net; 47 lower-traffic warnings remain for a future pass.)
...
- Verified via testing_agent (iteration 280): dot measured at exactly (36%,47%)/(49%,47%), toggle on/off works, logo upload + 35/40/45/50 size buttons + preview-left layout all intact.
...
## 2026-06-28 (fix #3) — Uncategorized still 0: add fallback strategy + diagnostics
- Prod still imported only 53 (uncategorized pull returned 0). Some Zoho orgs return 0 from the dedicated /banktransactions/uncategorized endpoint even when uncategorized lines exist — they surface only via /banktransactions?account_id=X&status=uncategorized.
...
## 2026-06-28 — Regenerate Invoice + Invoice pre-push Preview
- Regenerate Invoice: create_invoice_for_delivery(force=True) updates the existing Zoho invoice IN PLACE (same number); on Zoho rejection (paid/partial/credits) auto-voids + recreates; if even void is rejected → InvoiceNotRegenerableError (friendly 400). New zoho_service.regenerate_delivery_invoice(). Endpoints: POST /api/distributors/{d}/deliveries/{id}/regenerate-invoice and POST /api/invoices/{invoice_id}/regenerate (resolves invoice→delivery mapping). Management roles only (CEO/Admin/System Admin/Director/VP/Head of Business/National Sales Head/Regional Sales Manager). UI: Regenerate button + confirm dialog on Delivery detail AND per-row button on Invoices list.
- Invoice Preview: GET /api/distributors/{d}/deliveries/{id}/invoice-preview (no Zoho needed) computes line items (qty×rate, disc%, discount, net), subtotal, total discount, net taxable; GST noted as Zoho-applied. UI: 'Preview & Generate' opens dialog with table + 'Confirm & Push to Zoho' (disabled when missing agreed prices or net<=0).
- Verified iteration_261 (11/11 backend pytest; invoices-list regen button verified). DistributorDetail dialogs wired (E2E blocked only by reversed seed delivery). Live Zoho behavior needs prod redeploy.

## 2026-06-28 — Promotional stock-out crate reservation fix (reserved 1 instead of 12)
- RCA: promo (Promotional Dispatch) stored line-item quantity in CRATES + units_per_package, but the shared reservation/deduction/dashboard logic expects the regular Stock-Out convention (quantity in BOTTLES + packaging_units). So a 1-crate (12-bottle) promo reserved/deducted only 1.
- Fix (routes/promo_dispatch.py create): normalized to regular convention — quantity = crates × units_per_package (bottles), packaging_units = upp, packages = crates, unit_price = per-bottle (value unchanged: 12×10 = 1×120). Availability demand and header total_quantity now in bottles. Confirm/complete/batch-allocation/_adjust_stock read the stored bottle quantity → correct reserve & deduction.
...
## 2026-06-29 — BUGFIX: Promo challan printed crates as "1 Bottle"
- Root cause: the Promotional Stock-Out form's packaging dropdown was driven ONLY by packaging_config.promo_stock_out, which is empty for several SKUs (incl. the 4 configured Nyla SKUs). With no crate selectable, units_per_package stayed null → create_promo_dispatch defaulted to 1 → line stored quantity=1 (per-crate price), so the Zoho challan printed "1 Bottle" instead of e.g. 24/12 bottles.
- Fix: PromoDispatchSection.jsx now resolves packaging via fallback chain promo_stock_out → stock_out → packaging master (promoPacksFor helper), used in onSelectSku + the line render. A crate is now always selectable, units_per_package is captured, and backend normalizes quantity to bottles + per-bottle unit_price.
- Verified iteration_268 (backend 6/6; frontend dropdown now shows [Carton-6, Crate-12] for Nyla 600 Silver; 1 crate → quantity=12, packages=1, packaging_units=12). No real Zoho push.
- NOTE: This was reported on PRODUCTION. The fix is in PREVIEW — user must REDEPLOY. Existing wrong challans must be recreated. Production also benefits most once SKU stock_out packaging exists (fallback prefers stock_out, which works without the UOM migration).

## 2026-06-29 — BUGFIX (round 2): Promo challan "1 Bottle" — auto-promo path + backend safety net
- The "crate prints as 1 Bottle" bug had a SECOND surface beyond the manual form: the Delivery Order auto-promo (place_order). Added backend safety net _resolve_units() in routes/promo_dispatch.py create_promo_dispatch: when a line names a packaging_type (id/name) but lacks units_per_package, it resolves the unit count from the SKU's packaging_config (promo_stock_out → stock_out → master) so a crate never collapses to 1 bottle. Negative case (no matching pack) safely stays qty=1 (no fabrication).
...
## 2026-06-30 — Account contract: allow re-upload after approval (parity with Lead proposal)
- AccountDetail.js: new canUploadNewContract flag + generalized upload block so the "Upload Contract" control shows at ANY status (approved/rejected/changes_requested/pending_review), not just changes_requested. Dynamic label ("Upload New Contract" when approved, "Upload Revised Contract" when changes requested, else "Replace Contract") + a note that re-uploading an approved contract resets the approval cycle. data-testid replace-contract-input.
- Backend already supported it (server.py upload_account_contract versions + resets status to pending_review unless prior was changes_requested→revised, and creates a fresh approval task) — no backend change needed.
- Verified iteration_274 (backend 100%: re-upload of approved → v2 pending_review + approval task; changes_requested → revised; frontend 100%: control visible across statuses, correct labels, end-to-end UI re-upload of an approved contract).

## 2026-06-30 — Revenue Analytics: GOA (state) revenue undercount fixed
- Bug: Revenue Analytics state total for GOA was lower than the true sum of completed GOA stock-out invoices.
- Root cause (routes/revenue_analytics.py _aggregate): invoice→account matching tried only the FIRST truthy of account_id (human code) vs account_uuid against both lookup maps. Invoices whose code didn't resolve but whose UUID did were dropped into 'Uncategorised', undercounting the real state. Invoices store no city/state of their own, so account match is the only attribution path.
- Fix: new _match_account() helper tries BOTH account_id and account_uuid, each against by_code AND by_uuid maps, then name fallback. Shared by _aggregate and the new diagnostic so they can never disagree.
- New endpoint GET /api/reports/revenue-state-diagnostic?state=Goa&time_filter=...&only_target=bool — per-invoice reconciliation: target_state_net, unmatched_invoice_count/net, and rows (invoice_no, account_id, account_uuid, matched, attributed_state, net). Projected fields + only_target trim for large prod tenants.
- Verified iteration_275 (backend 100%, 11 tests): analytics(group_by=state) total == diagnostic target_state_net for the same state (invariant by construction). Preview seed only has Telangana; production multi-state data validates the same invariant. NEEDS REDEPLOY to take effect on production.

## 2026-06-30 — Invoices module: Void invoice + Group by customer/date
- VOID INVOICE (option a): new POST /api/invoices/{invoice_id}/void (routes/invoices.py). CEO/Admin only, double-confirm (type VOID + optional reason). Voids the invoice in Zoho Books (number kept, marked VOID for audit), decrements the account's running outstanding_balance if outstanding_counted, removes the local invoice doc, and recalcs account total_* aggregates. If the Zoho void fails -> 502 and local copy is LEFT INTACT (no changes). UI: ↺ Void action in InvoicesList table + card rows with AlertDialog double confirmation. Verified iteration_276 (8/8, 100%, Zoho mocked — no live calls).
- GROUP BY (InvoicesList.js): client-side grouping selector — No grouping / Group by customer / Group by date — with expandable/collapsible group headers showing per-group Gross/Net/Outstanding subtotals and count. Works in both table and card views, on desktop + mobile filter bars. All server-side filters (period/territory/state/city/status/search) are honored; grouping applies over the loaded page. Verified via screenshot.
...
## 2026-07-04 — Design Request (New) Detail: "Control Room" bento redesign
- Completed the DesignRequestNewDetail.js overhaul per /app/design_guidelines.json. Discarded the old white-card stack for a 12-column bento: left col-span-8 (Requirement, Production, Work Versions, Status History, Comments) + a sticky lg:col-span-4 "Command Center" right rail.
- Command Center hero = dark control panel (bg-stone-900) with "CURRENT STATE" + glowing pulse dot (state color) and big color-coded transition CTAs (h-14 block buttons) driven by actionMeta() intent mapping: emerald=progress/approve, amber=inputs/rework, indigo=review, rose=reject, violet=production. Blocked transitions render disabled with lock icon + tooltip. Removed the old flat inline Action bar (avoids duplicate action-*-btn testids). Meta moved to a compact Details card below the hub.
- All existing logic preserved verbatim (transitions/dialogs, file upload/preview/delete, versions approve/revert/delete/comments/@mentions, est-date popover, urgent toggle, send-for-printing, delete request). New testids: mr-action-hub, mr-hub-state, mr-meta-card. Verified via screenshot + live transition (Submitted → Inputs Needed) confirming CTAs + full-page state propagation.

## 2026-07-05 — Transition business-rules (guards) framework: enhancements
- Context: a generic guard engine already existed (utils/sm_helpers.evaluate_guards + FIELD_REGISTRY/OPERATORS_BY_TYPE + a builder UI in Admin→State Machines + runtime enforcement in available-transitions/transition). This session extended it.
- (b) Fixed: /api/state-machines/fields/catalog now resolves enum options for design_requests_new too (was marketing_requests-only) — request type & department dropdowns + applies_when now populate in the builder for that workflow. Added lead.status option resolution (distinct lead statuses + standard pipeline defaults).
- (c) New guardable DERIVED fields (added to FIELD_REGISTRY marketing_requests, inherited by design_requests_new): `approved_versions` (list — "has an APPROVED work version"), `lead.status` (enum), `lead.logo_url` (text — "linked lead has a logo"), `lead.city` (text). New async `augment_doc_for_guards(doc, tenant_id)` in sm_helpers computes approved_versions + fetches nested lead.* before guard evaluation; wired into available_transitions + trigger_transition in BOTH design_requests_new.py and marketing_requests.py.
- (d) Builder UX (StateMachines.js): each transition now shows a plain-English guard preview ("In plain English: this action is blocked unless Work versions has at least 1.", data-testid guard-preview-{idx}) and a collapsed amber "Blocked unless: …" chip (guard-summary-{idx}); new guardSummary() helper (strips "(N)" for readability).
- Also removed the "Filter by state:" label on the Design Requests - New list page (DesignRequestsNew.js).
- Verified: testing_agent iteration_285 — backend 4/4 pytest (catalog fields+options; end-to-end enforcement on DRN-2026-0001: guard on close → guards_ok=false + 400 block message; approved_versions guard likewise) and frontend (builder field dropdown lists new fields, enum options populate, preview+chip render, label removed). SM snapshotted & restored (no residual guards). New test: /app/backend/tests/test_iteration_285_guard_framework.py.

## 2026-07-05 — Deployment fix: removed ActiveMQ + non-blocking startup
- Production deploy was failing with a K8s readiness timeout ("deployment failed to become ready"). Root-cause hardening applied at code level (no Docker/manifest changes).
- REMOVED the ActiveMQ / Amazon MQ (stomp) invoice-subscriber integration entirely (user: "not using anymore"): deleted /app/backend/mq_subscriber.py; removed server.py globals (MQ_AVAILABLE/mq_subscriber/start/stop), the import block, the MQ start/stop calls in startup/shutdown, and the endpoints GET /api/mq/status, POST /api/invoices/webhook, GET /api/activemq/status + InvoiceWebhookPayload model. No frontend referenced these.
- MADE STARTUP NON-BLOCKING (key readiness fix): startup_event previously `await`ed routes.accounting_transactions.ensure_indexes() (a Mongo index build) on the critical startup path — a slow/unreachable Atlas DB could delay "Application startup complete" and thus the readiness probe. Now it does asyncio.create_task(_ensure_txn_indexes_bg()); the other startup handler (init_master_locations) was already create_task'd. Uvicorn now serves + passes /api/health immediately. shutdown just closes the mongo client.
- Verified: testing_agent iteration_286 — 13/13 backend pytest passed (health probes 200, login, removed endpoints gone, core reads leads/design-requests-new/invoices/accounting-transactions 200, guard-framework regression 200). Backend boots with no import/MQ errors. Test: /app/backend/tests/test_iteration_286_activemq_removal.py. NOTE: actual K8s production deploy cannot be validated from the sandbox — user must RE-DEPLOY.

## 2026-07-05 — Design Requests - New: full list/dashboard redesign (design_agent blueprint)
- User: "completely redesign the look and feel… options are scattered everywhere, lot of pills, lot of data… make it contemporary, well organized, user friendly" (keep Kanban as-is).
- design_agent produced /app/design_guidelines.json ("Swiss & High-Contrast" + emerald/zinc). Implemented in DesignRequestsNew.js (full rewrite, data logic unchanged):
  - Consolidated the 4 scattered control bands (header + queue tabs + 8 status pills + 4-field filter block) into ONE "command strip": segmented Queue switcher (All/Raised By Me/Assigned To Me + counts), a borderless Search, a Status popover (color-dot list + counts, single-select — replaces the 8 pills), and an Advanced-Filters popover (Request Type / Assigned Team / Requested By native selects + Clear all + active-count badge). View toggle (List/Kanban) moved into the header next to Export CSV / New Request.
  - List rows cleaned up: 2-line Request cell (title + mono number + age + snippet), compact StatusBadge (dot + label), consolidated people/due-date, subtle left-accent (rose=urgent micro-badge, blue dot=new) instead of full-row color washes. Added Inbox empty state + skeleton loading. Kanban untouched.
  - New testids: queue-segment-control, status-filter-trigger, status-option-all, advanced-filters-trigger, view-toggle-list-kanban, request-list-row-{id}, mr-new-dot-{id}; kept mr-search-input, mr-filter-type/dept/requestedby, mr-view-list/kanban, mr-export-btn, new-marketing-request-btn, tab-{key}, state-chip-{key}, mr-sort-{field}.
- Verified: testing_agent iteration_287 — 100% frontend (13/13) at 1920px + 390px. No console/compile errors. Kanban confirmed unchanged.

## 2026-07-05 — Kanban scroll arrows + Marketing→Design Requests migration
- KANBAN ARROWS (RequestKanbanNew.jsx): added floating left/right circular scroll buttons (kanban-scroll-left / kanban-scroll-right) that smooth-scroll the board ~80% width; auto-hide at each edge via scroll listener + updateScrollState. Board remains drag/scroll as before.
- MIGRATION Marketing Requests → Design Requests - New: new admin-only idempotent endpoint POST /api/design-requests-new/migrate-from-marketing (dry-run default, ?commit=true writes). Scoped to current tenant. Copies ALL fields (type, requirement, assignee, requestor, dates, comments, versions, references, logo, production, links, timestamps); repoints state_machine_id/name and refreshes current_state_label/color from the DRN state machine; sets DRN-only fields (board_rank, is_urgent=false, lead_* from source/null, status_history migration entry). Keeps original MR-2026-xxxx numbers; stores migrated_from_marketing_request_id (+request_number) for idempotency. Copies referenced file metadata from marketing_request_files → design_requests_new_files (same id + object-storage path, NO re-upload) so logos/attachments stay viewable. _MIGRATION_STATE_MAP is identity (MR & DRN share the same 8 state keys). Originals left untouched.
- Admin UI (DesignRequestsNew.js): header "Migrate" button (open-migration-btn, gated by ADMIN_ROLES) → MigrationDialog shows dry-run summary (to_migrate / already_done / files + state breakdown + unmapped warning) then a commit button; shows success result and refreshes list/counts.
- Ran in PREVIEW for nyla-air-water: 19 migrated, 20 files copied, idempotent (re-run 0). Verified testing_agent iteration_288 — backend 5/5 pytest + frontend 100%: migrated detail pages render Action Hub with valid CTAs (SM repointed), files/comments/links preserved, counts.all=20, non-admin 403. Test: /app/backend/tests/test_design_requests_new_migration.py.
- PRODUCTION: user must re-deploy, then click "Migrate" (dry-run → commit) on the Design Requests - New page to migrate production marketing_requests (separate Atlas DB).
- STILL PENDING (confirmed earlier, not yet built): "Lead-only" request-type flag to hide Neck Tags / Request Bottle Design Concept / Request Physical Sample from the Design Request create form (manageable toggle in Request Type Masters).

## 2026-06 — Print Request creation from Customer Branding
- FEATURE (LeadBottleDesigns.js): On each Design Request row in the Customer Branding section, a "Print Request" button (create-print-request-btn-<id>) now shows ONLY when the request state is Final Approved (final_approved / production_in_progress / production_completed). Row markup restructured from a single <button> to a div so the nested action button is valid HTML. Row-click still opens the design request detail page.
- Wired to CreatePrintRequestDialog (fixed DatePicker import: named export from ui/date-picker). Dialog pre-fills Total Monthly Volume from the lead's current_volume (returned by GET /api/leads/{id}/bottle-designs), plus Starting Monthly Volume, Initial Order Quantity (required >0), Requested Delivery Date (required), Notes. Submits POST /api/print-requests.
...
## 2026-06 — Unified Print Request fields, order-only banner, mandatory rules & city grouping
- Unified fields across ALL print-request surfaces (CreatePrintRequestDialog, SendForPrintingDialog, PrintRequestDetail edit): Total Monthly Volume (Future Potential), Initial Monthly Quantity (=starting_monthly_volume), Initial Order Quantity, Requested Delivery Date, Notes. SendForPrinting keeps optional team+vendor.
- New shared PrintRequestOrderBanner.jsx (amber) on all create/edit dialogs: "Raise a Print Request only for confirmed customer orders. Do not create one for design reviews or sample approvals."
- Mandatory: Initial Order Quantity, Initial Monthly Quantity, Requested Delivery Date (frontend toasts + backend enforcement). Backend create now 400s without starting_monthly_volume; PATCH persists initial_order_quantity (syncs quantity), starting_monthly_volume, total_monthly_volume (PrintRequestUpdate extended).
- Relabel: "Total Monthly Volume" → "Total Monthly Volume (Future Potential)"; detail-page InfoRows now read "Initial Monthly Quantity" / "Total Monthly Volume (Future Potential)" for parity.
